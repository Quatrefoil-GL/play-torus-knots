  ...
  :files $ {}
    |app.comp.container $ %{} 'FileEntry
      :defs $ {}
        |field $ %{} 'CodeEntry (:doc |)
          :code $ quote
            defn field (value key) $ option:unwrap-or (get value key) nil
          :examples $ []
          :schema $ :: 'Dynamic
                ...
                scene ({})
                  group
                    ...
                    sphere $ {} (:radius 2) (:width-segments 10) (:height-segments 8)
                      ...
                      :event $ {}
                        :click $ fn (e d!) (.!requestFullscreen (ffi-object js/document.body))
          ...
          :code $ quote
            defn knots-fn (ratio factor)
              let
                  ...
                  r $ * 0.5 (- b a)
                  center $ + a r
                  t1 $ * ratio speed (js-number (.-PI (ffi-object js/Math)))
                  point $ []
                    + center $ * r (js-number (js/Math.cos t1))
                    , 0
                      * r $ js-number (js/Math.sin t1)
                  t2 $ * r-speed ratio (js-number (.-PI (ffi-object js/Math)))
                &q*
                  &q*
                    [] 0 0 (js-number (js/Math.cos t2)) (js-number (js/Math.sin t2))
                    , point
                  [] 0 0
          ...
          :code $ quote
            defn main! () (load-console-formatter!) (inject-tree-methods)
              set-perspective-camera! $ {} (:fov 40) (:near 0.1) (:far 100)
                :position $ [] 0 0 8
                :aspect $ / (js-number (.-innerWidth (ffi-object js/window))) (js-number (.-innerHeight (ffi-object js/window)))
              let
